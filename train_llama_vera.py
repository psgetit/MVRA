...
from moe_vera import MoEVeRA
from utils import plot_gradients, plot_values
from custom_optimizer import AdamW, SGD, AdamWr, SGDr
from utils import init_lora_A, init_lora_B, init_gate, wrap_print_function, init_scaling_b, init_scaling_d
...
def load_model(pretrained_model_path, lora_r, lora_alpha, lora_dropout, lora_modules, number_experts, top_k, torch_type,
               descent_strategy):
    ...
    for name, module in model.named_modules():
        if isinstance(module, peft.tuners.lora.LoraLayer):
            ...
            # init_lora_A(module.lora_A)
            # init_lora_B(module.lora_B)
            init_gate(module.gate)
            for i in range(number_experts):
                print('building expert {}'.format(i))
                init_scaling_d(module.scaling_d[i])  # 修改这里
                init_scaling_b(module.scaling_b[i])  # 修改这里
...
def train(train_loader, val_loader, test_loader, model, per_gpu_batch_size, accumulated_steps, optimizer, scheduler,
            ...
            if total_steps % accumulated_steps == 0:
                if max_gate_grad_norm is not None:
                    torch.nn.utils.clip_grad_norm_(
                        [param for name, param in model.named_parameters() if name.find('gate.') != -1],
                ...
                print(
                    f"[{time_str}] Step:{'%4d' % gradient_update_steps}/{total_gradient_update_steps}; Epoch:{'%2d' % epoch}({fmt % (per_gpu_batch_size * (total_steps % len(train_loader)))}/{per_gpu_batch_size * len(train_loader)}); Loss:{loss_item};\tLR:{[g['lr'] for part in optimizer for g in optimizer[part].param_groups]}\n",
                    end="")
                loss_item = 0.0
...
def main(optimize_strategy, descent_strategy):
    ...
    print('start base model loading')
    # model configuration
    ...
    per_gpu_batch_size = candidate_configs[optimize_strategy]["per_gpu_batch_size"][
        max_window_length + max_generated_tokens]
    accumulated_steps = candidate_configs[optimize_strategy]["accumulated_steps"][
        max_window_length + max_generated_tokens]
    ...
    optimizer_expert = ({
        ...
        "sgd|default": SGD,  # 添加普通 SGD 选项
        "adamw|default": AdamW  # 确保 AdamW 也有默认选项
...
if __name__ == "__main__":
    ...
    descent_strategy = sys.argv[3]
    if descent_strategy not in ["riemannian", "moe-riemannian"]:
        descent_strategy = "default"
